...
class MtModelExporter(object):
    ...
    def _shouldExportNode( self, node ):
        ...
        if node.isHidden:
            return False
        if mtmaxconfig.lukasCompat and (self._isBoneNode( node ) and node.name == 'bone255'):
            return False
        return True
...
don't have an reference/original model specified as it will override the skeleton in the scene.''' )
        ...
        for i, primWorkingSet in enumerate(primWorkingSets.values()):
            ...
            for prim in primWorkingSet.primitives:
                ...
                mtmaxlog.debug( "trimming uvs" )
                prim.removeUnusedUvs( _progressCallback )
                ...
                if target.current.useTriStrips:
                    prim.generateTriStrips( _progressCallback )
