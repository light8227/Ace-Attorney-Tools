...
class MtModelImporter:
    ...
    def importPrimitive( self, primitive, primitiveJointLinkIndex, indexStream, vertexStream ):
        ...
        for j in range( 0, primitive.vertexCount ):
            ...
            for key, value in shaderInfo.inputsByName.items():
                for inputInfo in value:
                    ...
                    if key == 'Position':
                        pos = self.decodeInputToMaxPoint3( inputInfo, vertexStream )
                        if not mtmaxconfig.debugDisableTransform:
                            pos *= self.maxModelMtx
                        rt.append( maxVertexArray, pos )
                    elif key == 'Normal':
                        nrm = self.decodeInputToMaxPoint3( inputInfo, vertexStream )
                        if not mtmaxconfig.debugDisableTransform:
                            nrm = rt.normalize( nrm * self.maxModelMtxNormal )
                        rt.append( maxNormalArray, nrm )
                    elif key == 'Joint':
        ...
        if not target.current.useTriStrips:   
            ...
            a = indexStream.readUShort() - primitive.vertexStartIndex + 1
            b = indexStream.readUShort() - primitive.vertexStartIndex + 1
            flip = True
            j = 2
            while j < primitive.indexCount:
                ...
                if c == 0xffff:
                    ...
                    c = c - primitive.vertexStartIndex + 1
                    flip = not flip
                    if flip:
                        face = rt.Point3( c,b,a )  
                    else:
                        face = rt.Point3( a,b,c )
        ...
        hasUvs = len( maxUVPrimaryArray ) > 0
        meshName = self.metadata.getPrimitiveName( primitive.id )
        maxMesh = rt.Mesh(vertices=maxVertexArray, faces=maxFaceArray)
    ...
    def importSkeleton( self ):
        ...
        rootBone = None
        if mtmaxconfig.lukasCompat:
            rootBone = self.createMaxBone('bone255', rt.Matrix3(1), None)
            rt.setUserProp( rootBone, 'LMTBone', 255 )
        ...
        for i, joint in enumerate( self.model.joints ):
            ...
            elif rootBone is not None:
                maxParentBone = rootBone
            ...
            rt.setInheritanceFlags( maxBone, rt.BitArray(1,2,3,4,5,6) )
            rt.setUserProp( maxBone, 'LMTBone', joint.id )      
